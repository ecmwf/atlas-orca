    ...
    void fix() {
        ...
        if( core == 0. ) {
            if( I == I_master && J == J_master ) {
                if( verbose ) {
                    Log::info() << "Point {i,j} = {" << I-1 << "," << J-1 << "} --> {" << I_master-1 << "," << J_master-1 << "} should be marked as core" << std::endl;
                }
                core = 1.;
            }
        }
            ...
            if( I != I_master || J != J_master ) {
                if( verbose ) {
                    Log::info() << "Point {I,J} = {" << I << "," << J << "} --> {" << I_master << "," << J_master << "} should be marked as non-core" << std::endl;
                }
                core = 0.;
            }
        }
        if( lat != lat_master && J_master != 0 ) {
            if( verbose ) {
                Log::info() << "Point {I,J} = {" << I << "," << J << "} --> {" << I_master << "," << J_master << "} has wrong lat: [" << lat << " != " << lat_master << "]" << std::endl;
            }
            lat = lat_master;
        }
        if( lon != lon_master && J_master != 0 ) {
            if( verbose ) {
                Log::info() << "Point {I,J} = {" << I << "," << J << "} --> {" << I_master << "," << J_master << "} has wrong lon: [" << lon << " != " << lon_master << "]" << std::endl;
            }
            lon = lon_master;
        }
    }
...
public:
    ...
    void read( const std::string& uri, OrcaData& data ) {
        ...
        if ( trace.elapsed() > 1. && verbose_ ) {
            Log::info() << "Reading file " << file.path() << " took " << trace.elapsed() << " seconds."
                        << std::endl;
        }
            ...
            for( idx_t i = 0; i < ni; ++i ) {
                ...
                if( j > halo[S] ) {
                    ...
                    if( n_master != check_master[n] ) {
                        ...
                        Log::warning() << "    point  " << n << "\t" << PointIJ(i,j) << std::endl;
                        Log::warning() << "    master " << n_master << "\t" << master << std::endl;
                        Log::warning() << "    file   " << check_master[n] << "\t" << PointIJ(i_check,j_check) << std::endl;
                        Log::warning() << "---------------------------------------------------" << std::endl;
