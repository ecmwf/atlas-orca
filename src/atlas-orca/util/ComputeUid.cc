...
std::string compute_uid(const std::string &arrangement, const double lon[], const double lat[], int nx_halo, int ny_halo, const int32_t halo[]) {
    ...
    if( arrangement == "F" ) { valid_arrangement = true; }
    if( arrangement == "T" ) { valid_arrangement = true; }
    if( arrangement == "U" ) { valid_arrangement = true; }
    if( arrangement == "V" ) { valid_arrangement = true; }
    if( arrangement == "W" ) { valid_arrangement = true; }
    if( arrangement.empty()) { valid_arrangement = true; }
