...
CASE( "test spec" ) {
    ...
    auto registered = [&]( const std::string& uid_or_name ) {
        return grid::SpecRegistry::has( uid_or_name );
    };
    ...
    for( const auto& grid: grids ) {
        ...
        for( const auto& P: arrangements ) {
            ...
            Log::info() << std::setw(11) << std::left << name << "    " << uid(name) << "    " << dimensions(name) << std::endl;
